...
const StudioCard = ({
  ...
  color: string;
  isBeta?: boolean;
  supportedScopes?: ('document' | 'workspace')[];
  currentScope?: 'document' | 'workspace';
  onClick?: () => void;
}) => {
  const isSupported =
    !supportedScopes || !currentScope || supportedScopes.includes(currentScope);
  ...
  return (
    <div
      ...
      style={{
        ...
        filter: isSupported ? 'none' : 'grayscale(100%)',
      }}
      onClick={() => {
        if (isSupported && onClick) onClick();
      }}
    >
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          {icon && (
            <div
              style={{ color: 'var(--affine-text-secondary)', display: 'flex' }}
            >
              {icon}
            </div>
          )}
...
export const EditorStudioPanel = ({ editor }: EditorStudioPanelProps) => {
  ...
  const [currentScope, setCurrentScope] = useState<'workspace' | 'document'>(
    'document'
  );
  const [activeTab, setActiveTab] = useState<'base' | 'specialized'>('base');

  const [generatedDocs, setGeneratedDocs] = useState<Record<string, string>>(
    () => {
      try {
        return JSON.parse(
          localStorage.getItem('lexior_generated_docs') || '{}'
        );
      } catch {
        return {};
      }
  ...
  const organizeGeneratedDoc = (docId: string) => {
    ...
    try {
      ...
      const organizeService =
        workspaceService.workspace.scope.get(OrganizeService);
      ...
      if (!analyseFolder) {
        ...
        const alreadyLinked = existingChildren.some(
          child => child.data$.value === docId && child.type$.value === 'doc'
        );
        if (!alreadyLinked) {
          analyseFolder.createLink(
            'doc',
            docId,
            analyseFolder.indexAt('after')
          );
        }
      }
    } catch (e) {
  ...
  const serverService =
    workspaceServerService.server || defaultServerService.server;
  ...
  useEffect(() => {
    activeJobs.forEach(async job => {
      const currentGeneratedDocs = JSON.parse(
        localStorage.getItem('lexior_generated_docs') || '{}'
      );
      if (
        job.status === 'finished' &&
        job.summary &&
        !currentGeneratedDocs[job.id]
      ) {
        ...
        try {
          ...
          if (docId) {
            const docsService =
              workspaceService.workspace.scope.get(DocsService);
            const titleMap: Record<string, string> = {
              ...
              appel_offres: "Traitement d'Appel d'Offres",
              due_diligence: 'Due Diligence M&A',
              hr_compliance: 'Conformité RH',
              ao_go_no_go: 'Analyse Go/No-Go',
              ao_requirements: 'Matrice des Exigences',
              ao_risks: 'Risques & Pénalités (AO)',
              ao_outline: 'Trame de Réponse',
              ao_final_validation: 'Validation de la Réponse',
            };
            const title =
              titleMap[job.actionType] || `Analyse Lexior : ${job.actionType}`;
            ...
            if (job.actionType === 'infographic') {
              setTimeout(async () => {
                try {
                  const { AIProvider } =
                    await import('@affine/core/blocksuite/ai/provider');
                  if (AIProvider.actions.createImage) {
                    ...
                    if (imageUrl) {
                      ...
                      if (imageBlob) {
                        const sourceId =
                          await workspaceService.workspace.docCollection.blobSync.set(
                            new File([imageBlob], 'infographie.png', {
                              type: imageBlob.type,
                            })
                          );
                        const doc =
                          workspaceService.workspace.docCollection.getDoc(
                            docId
                          );
                        if (doc) {
                          doc.load();
                          const noteBlock =
                            doc.getBlocksByFlavour('affine:note')[0];
                          if (noteBlock) {
                            doc.addBlock(
                              'affine:image',
                              { sourceId: sourceId },
            ...
            if (job.actionType === 'presentation') {
              ...
              setTimeout(() => {
                ...
                if (host) {
                  import('@affine/core/blocksuite/ai/slides').then(
                    ({ PPTBuilder }) => {
                      PPTBuilder(host)
                        .process(job.summary)
                        .catch(console.error);
  ...
  const handleAction = async (actionName: string) => {
    ...
    try {
      ...
      const docId =
        currentScope === 'document' ? editor.doc.id : 'workspace-scope';
    ...
    <div className={styles.root}>
      ...
      <div className={styles.pinToggle}>
        <Switch
          ...
          onChange={checked =>
            setCurrentScope(checked ? 'workspace' : 'document')
          }
        />
        <div
          style={{
        ...
        >
          ...
          <span
            style={{ fontSize: '11px', color: 'var(--affine-text-secondary)' }}
          >
            {currentScope === 'workspace'
              ? 'Analyse étendue à tout le dossier'
              : 'Analyse limitée à ce document'}
          </span>
        </div>
      </div>
      ...
      >
        ...
        <div
          ...
          style={{
            cursor: 'pointer',
            fontWeight: activeTab === 'specialized' ? 600 : 400,
            color:
        ...
        <>
          <div className={styles.section}>
            ...
            <div className={styles.grid}>
              ...
              <StudioCard
                ...
                color="#862E9C"
                isBeta
                onClick={() => handleAction('infographic')}
              />
            </div>
          </div>
          ...
          <div className={styles.section} style={{ marginTop: '24px' }}>
            <div className={styles.sectionTitle}>
              Actions Juridiques (Dossier)
            </div>
            <div className={styles.grid}>
              <StudioCard
                title="Valider Contrat"
                icon={<SignOutIcon />}
                bgColor="#E6FCF5"
                color="#087F5B"
                onClick={() => handleAction('validate_contract')}
              />
              <StudioCard
                ...
                bgColor="#FFF5F5"
                color="#C92A2A"
                onClick={() => handleAction('risk_analysis')}
              />
              <StudioCard
                ...
                bgColor="#F4FCE3"
                color="#5C940D"
                onClick={() => handleAction('dossier_summary')}
              />
              <StudioCard
                title="Rédiger Clause"
                icon={<FileIcon />}
                bgColor="#E7F5FF"
                color="#1864AB"
                onClick={() => handleAction('draft_clause')}
              />
            </div>
          </div>
        </>
      ...
      {activeTab === 'specialized' && (
        <div className={styles.section}>
          <div className={styles.sectionTitle}>
            Lexior Store (Plugins installés)
          </div>
          <div
            style={{
              color: 'var(--affine-text-secondary)',
          ...
          >
            Déployez ces outils MCP configurés spécifiquement pour votre
            organisation.
          </div>

          <McpAccordion
            title="Appels d'Offres (M&A / Public)"
            description="Suite complète pour analyser et répondre aux appels d'offres complexes."
            icon={<FileIcon />}
          >
            <StudioCard
              ...
              onClick={() => handleAction('ao_go_no_go')}
            />
            <StudioCard
              title="Matrice des Exigences"
              icon={<ViewLayersIcon />}
              bgColor="#EBFBEE"
              color="#2B8A3E"
              supportedScopes={['document', 'workspace']}
              currentScope={currentScope}
              onClick={() => handleAction('ao_requirements')}
            />
            <StudioCard
              title="Risques & Pénalités"
              icon={<AiIcon />}
              bgColor="#FFF5F5"
              color="#C92A2A"
              supportedScopes={['document']}
              currentScope={currentScope}
              onClick={() => handleAction('ao_risks')}
            />
            <StudioCard
              title="Trame de Réponse"
              icon={<FileIcon />}
              bgColor="#E7F5FF"
              color="#1864AB"
              supportedScopes={['document', 'workspace']}
              currentScope={currentScope}
              onClick={() => handleAction('ao_outline')}
            />
            <StudioCard
              title="Validation de Réponse"
              icon={<CardPanelIcon />}
              bgColor="#F3F0FF"
              color="#6741D9"
              supportedScopes={['document', 'workspace']}
              currentScope={currentScope}
              onClick={() => handleAction('ao_final_validation')}
            />
          </McpAccordion>

          <McpAccordion
            title="Due Diligence M&A"
            description="Audit automatisé de données dans le cadre de fusions et acquisitions."
            icon={<ViewLayersIcon />}
          >
            <StudioCard
              ...
              currentScope={currentScope}
              onClick={() => handleAction('due_diligence')}
            />
            <StudioCard
              title="Détection des Passifs"
              icon={<AiIcon />}
              bgColor="#F8F9FA"
              color="#495057"
              supportedScopes={['workspace']}
          ...
          <McpAccordion
            title="Conformité RH"
            description="Outils d'audit pour les contrats de travail et la réglementation sociale."
            icon={<CardPanelIcon />}
          >
            <StudioCard
              title="Audit Contrat"
              ...
              supportedScopes={['document']}
              currentScope={currentScope}
              onClick={() => handleAction('hr_compliance')}
            />
            <StudioCard
              title="Bilan Social"
              icon={<FileIcon />}
              ...
              {job.status === 'pending' || job.status === 'running' ? (
                ...
                <>
                  <span style={{ color: 'red' }}>
                    Action échouée : {job.actionType}
                  </span>
                </>
              ) : (
                <>
                  <span style={{ color: 'green', fontWeight: 600 }}>
                    Action terminée : {job.actionType}
                  </span>
                </>
              )}
            ...
            {job.summary && (
              <div
                ...
                style={{
                  marginTop: '12px',
                  padding: '16px',
                  background: 'var(--affine-background-secondary)',
                  borderRadius: '8px',
                  border: '1px solid var(--affine-border-color)',
                }}
              >
                {generatedDocs[job.id] &&
                generatedDocs[job.id] !== 'generating' &&
                generatedDocs[job.id] !== 'error' ? (
                  <div
                    style={{
                      display: 'flex',
                      flexDirection: 'column',
                  ...
                  >
                    ...
                    >
                      Le rapport d'analyse a été généré et sauvegardé dans votre
                      espace.
                    </div>
                    <Button
                      variant="primary"
                  ...
                  >
                    <ResetIcon className={styles.spinner} /> Création
                    automatique du document en cours...
                  </div>
                ) : (
                  <div
                    style={{
                  ...
                  >
                    ...
                    >
                      Une erreur est survenue lors de la création automatique.
                        ...
                        try {
                          ...
                          if (job.actionType === 'infographic') {
                            try {
                              ...
                              if (AIProvider.actions.createImage) {
                                toast(
                                  "Génération de l'infographie en cours..."
                                );
                                const imageUrl =
                                  await AIProvider.actions.createImage({
                                    input:
                                      'Generate an infographic based on this text:\n' +
                                      job.summary,
                                    workspaceId: workspaceService.workspace.id,
                                  });
                                if (imageUrl) {
                              ...
                              console.error(
                                'Failed to generate image automatically',
                                e
                              );
                            }
                          }
                          ...
                          if (docId) {
                            ...
                            const title =
                              titleMap[job.actionType] ||
                              `Analyse Lexior : ${job.actionType}`;
                            await docsService.changeDocTitle(docId, title);

                            if (job.actionType === 'presentation') {
                              docsService.list.setPrimaryMode(
                                docId,
                                'edgeless'
                              );
                              saveGeneratedDoc(job.id, docId);
